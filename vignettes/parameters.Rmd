...
`disturbance_mean_interval` is the mean disturbance interval.  

Both `strategy` and `strategy_list` functions as well as the functions
that utilise them (such as `individual_list`) require an argument
known as `birth_rate_list` to set the birth rates of each species.

This object is a list that contains either constant or variable birth rates
for each species. For constant birth rates we simply need to provide a constant 
numeric value, wheras for variable birth rates we supply a list that contains a 
set of x and y control points that will be used to interpolate the birth rate 
spline. For example:
```{r}
x <- seq(0, 200)
birth_rates <- list(
...
```
The other examples in this document will simply set a constant birth rate of 
`1.0` for the one species present.

The `strategy_default` member of a `Parameters` object is a Strategy:
...
```{r}
s <- strategy(trait_matrix(0.1, "lma"), p, birth_rate_list=1.0)
```
...
ss <- strategy_list(lma, p, birth_rate_list=1.0)
length(ss)
```
...
```{r}
pp <- individual_list(lma, p, birth_rate_list=1.0)
